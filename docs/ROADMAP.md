...
1. **Main-ship centered.** The main ship is a unique INSTANCE (not stackable), the emotional
   ...
   > future-only).
   > **REFINEMENT (2026-07-02, fleet-group direction — see `DEV_LOG.md` 2026-07-02 +
   > `TRADE_FLEET_0A_FLEET_ADDENDUM.md`):** ships operate as a **FLEET GROUP** (≈6–8 ships) — the
   > group is the **movement / docking / location / activity owner**; member ships travel, arrive,
   > dock, and enter activities **together**. The individual ship owns hull/stats/ship-skills, its
   > volume cargo hold + ship-bound cargo, and up to **8 captain** assignments; the fleet's ability
   > is an aggregate over members. Ships within one active group do **NOT** move independently
   > (supersedes the earlier independent-ship model). Additional ships are **built from activity
   > rewards, not bought.** The legacy `fleets` table is a distinct legacy concept and must **not**
   > be overloaded for the group (a distinct `fleet_groups`-style identity is required).
...
| 10 ⏳ | Trading (buy low / travel / sell high; **volume-only (m³)** ship-bound cargo, route danger) | **designed, NOT built.** **FIXED product direction 2026-07-02** (see `DEV_LOG.md` 2026-07-02 + `TRADE_FLEET_0A_IMPACT_AUDIT.md` + `TRADE_FLEET_0A_FLEET_ADDENDUM.md`): **volume-only per-ship cargo (m³ canonical; NO kilograms/mass/dual-cap in V1), ship-bound cargo (never pooled).** **FLEET-GROUP model:** ships operate as a **fleet group** (≈6–8 ships) that is the **movement/docking/location/activity owner** and travels/docks/trades **together** (ships do NOT move independently within a group); each ship keeps its hull/stats/skills + volume hold + ≤8 captains; fleet ability = aggregate over members; **fleet cargo total is a derived read** over per-ship holds. **Additional ships are BUILT from activity rewards, not bought** (no ship vendor / no credit purchase). Out of V1 scope: pooled/account cargo, remote market, ship-to-ship transfer, warehouses, auto-routes, P2P trade, dynamic supply/demand, cargo loss/insurance, mass/fuel mechanics. Retained: free-port eligibility, server-owned `market_offers`, lazy wallet, `trade_receipts` idempotency, per-offer allowance. Legacy `public.fleets` must **not** be overloaded for the group (needs a distinct `fleet_groups`-style identity). **Sequence:** PORT-ENTRY (done, mig `0072`) → **TRADE-FLEET-0A** (read-only impact audit, merged PR #66) → **TRADE-FLEET-0A-FLEET** (fleet-group addendum audit) → **TRADE-FLEET-0B** (explicit user-approved fleet-group + volume-cargo contract) → **TRADE-FLEET-0C** (coherent implementation slice) → **TRADE-MARKET-1** (server-authoritative market) → **TRADE-UI-1** (fleet + selected-ship market UI) |
