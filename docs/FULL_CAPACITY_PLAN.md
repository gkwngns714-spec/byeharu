...
### P13 — SHIELD: regenerating ship shields *(M — SHIELD-0 SHIPPED dark as mig 0191 + SHIELD-1 SHIPPED inert as mig 0195; SHIELD-2 + ACT-SHIELD remain)*
...
hull is 0 — the 0195 header's scope note). **SHIELD-2**: the out-of-combat regen home (riding
the 0191 partial index) + UI (shield bars next to hp) + the commission copy. **ACT-SHIELD** (the human flip, [D — all
...
| 20 | NANGUARD — fix both adapters' NaN guards **[E — found by the DECKS-3 hostile review]** | **queued**: the `x <> x` NaN-detect idiom in `calculate_expedition_stats` is a NO-OP in PostgreSQL — `'NaN'::float8 = 'NaN'::float8` is TRUE (PG deviates from IEEE 754 here), so the guard arm is UNREACHABLE and a knob mis-set to `"NaN"` WOULD poison the folded stats. TWO sites: the 0180 level knob (`captain_level_bonus_per_level`) and the 0196 affinity knob (`station_affinity_bonus` — 0196 inherited the shape byte-for-byte DELIBERATELY, 0180-parity over a mid-slice idiom fork). ONE slice fixes BOTH to the working `= 'NaN'::float8` idiom (the 0182 worldstate-knob precedent) via a fresh adapter re-create from its then-true head, updates the guard-shape prosrc self-asserts the 0180/0193/0196 lineage pins, and corrects the inherited comments |
