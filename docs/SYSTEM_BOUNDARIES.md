...
| `exploration_discoveries` | **Exploration** (sole writer: `exploration_scan`; DARK behind `exploration_enabled=false`) | owner |
...
| **Exploration** | exploration_discoveries | `command_exploration_scan(ship,req)` *(authenticated; the ONE client entry — 0083 wrapper idiom: auth → anti-probe flag gate → `mainship_resolve_owned_ship` → delegate)* → private `exploration_scan(player,ship,req)` *(service-role/internal; the SOLE `exploration_discoveries` writer)*. **DARK** behind `exploration_enabled=false` (0097): the writer rejects `feature_disabled` BEFORE any other read, and the wrapper gates identically. The writer reuses — never re-invents — the OSN command machinery DOWNWARD: `main_ship_space_command_receipts` idempotency (0055; replay-safe on (ship, request_id)), the S2 canonical lock order (ship → fleet → coordinate movement → presence), `mainship_space_validate_context` (requires settled `in_space`) + `mainship_space_assert_cross_domain_exclusion`, and the pure geometry leaf `osn_distance`. It reads `exploration_sites` DOWNWARD (hidden server-only data; a site is revealed only at the moment of discovery) and ACCRUES the pending bundle on its own state (`pending_bundle_json`, `secured_at` NULL) — deposit flows only through the existing activity-agnostic carrier (`movement_attach_cargo(…, 'exploration')` → `process_fleet_movements` → `Reward.grant`; wired in a later slice) — never a parallel deposit path | write exploration_sites (no runtime writer exists) · deposit anything (no inventory/base/reward/movement write; scan only accrues) · write another activity's tables · expose an undiscovered site's coordinates to a client · run while the gate is off |
