...
| `mining_extractions` | **Mining** (ONE owner system: `mining_extract` (0104) inserts — the LIVE sole insert path, insert-only · the FORTHCOMING slice-D `process_mining_securing` sets `secured_at`; DARK behind `mining_enabled=false` — the writer rejects before any read while false, so no row can exist today) | owner |
