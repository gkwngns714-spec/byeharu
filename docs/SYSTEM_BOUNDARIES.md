...
| `mining_extractions` | **Mining** (ONE owner system, two FORTHCOMING writer fns: the slice-C `mining_extract` inserts · the slice-D `process_mining_securing` sets `secured_at`; DARK behind `mining_enabled=false` — schema-only today, NOTHING writes it yet) | owner |
...
| **Mining** | mining_extractions *(schema-only today — 0103; NOTHING writes it yet)* | FORTHCOMING (each slice lands dark behind `mining_enabled=false`, seeded 0102): slice C `command_mining_extract(ship,req)` *(authenticated wrapper, 0083 idiom)* → private `mining_extract(player,ship,req)` *(service-role/internal; inserts extraction rows — REPEATABLE, no `unique (player_id, field_id)`; pacing = the per-(player, field) cooldown from the latest `created_at` vs `mining_extract_cooldown_seconds`, proximity via the `osn_distance` leaf vs `mining_extract_radius`, reusing the OSN command machinery DOWNWARD exactly as Exploration does: receipts idempotency, S2 lock order, validate-context settled `in_space`, cross-domain exclusion)*; slice D `process_mining_securing()` *(cron; flag-ignoring in-flight safety, 0100 posture)* deposits a pending bundle once the carrying ship settles SAFE by calling `Reward.grant('mining', extraction_id, player, base, bundle)` — **`reward_grant` remains THE sole secured-deposit owner** — then sets `secured_at`; slice E read surface reveals ONLY fields the player has extracted from. Bundles are ITEMS-ONLY (`{items[]}` drawn from the existing `ore`/`crystal`/`artifact_core` catalog rows) — mining NEVER lands anything in `base_resources` (no second landing path to the Base-owned scalars). Reads `mining_fields` DOWNWARD (hidden server-only data). Edges (all DOWNWARD, acyclic, the Exploration shape): Mining → OSN geometry/locks (read) · Main Ship (read) · Bases (read: deposit target) · Reward (grant); nothing calls into Mining. Forfeiture of pending value: DEFERRED (0100 posture — pending rows wait) | write mining_fields (no runtime writer exists) · deposit outside `Reward.grant('mining', extraction_id, …)` (never write inventory/base tables directly) · write base_resources at all (items-only bundles) · write another activity's tables · expose an un-extracted field's coordinates/composition to a client · extract while the gate is off |
