...
| **Trade Market** | trade_receipts, trade_relief_claims | `get_market_offers(ship)` *(read)*, `market_buy(ship,good,qty,req)`, `market_sell(ship,good,qty,req)` *(authenticated; idempotent on (ship,request_id))* — orchestrates buy/sell by fanning out one-directionally DOWNWARD to Wallet (debit/credit) + Trade Cargo (add/consume lots), reading `trade_goods` + the docked-location context (via the shared Main-Ship helper `mainship_resolve_docked_location`); writes only its own `trade_receipts`. **DARK**: every trade RPC is server-rejected while `trade_market_enabled=false`. The forthcoming **no-softlock relief** orchestrator (DARK behind `trade_relief_enabled=false`) is sited here too: it writes ONLY its own `trade_relief_claims` (per-player idempotent ledger) and grants the relief credit THROUGH `wallet_credit` — reusing the EXISTING downward fan-out (Wallet credit, Trade Cargo / Main Ship reads), so it introduces **no new edge and no cycle**, Wallet stays a downward leaf, and `player_wallet`'s sole-writer invariant holds (relief never writes `player_wallet` directly). Tunables (Reference/Config): `relief_credits`, `relief_cooldown_seconds`, `relief_max_lifetime_claims` | write ship_cargo_lots/player_wallet/main_ship_instances directly · second-write any table · run while the gate is off |
...
> `trade_market_enabled=false`. `trade_relief_claims` is Trade-Market-owned but **starts dark with no writer**
> (its sole writer is the forthcoming relief RPC, DARK behind `trade_relief_enabled=false`) — exactly as
> `player_wallet`/`trade_receipts` started in 0086; its future writer adds no new cross-system edge (relief credit
> flows through `wallet_credit`, so Wallet remains the sole `player_wallet` writer).
