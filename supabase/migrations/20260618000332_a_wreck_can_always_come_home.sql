...
begin
  for r in
    ...
    (1, 'process_combat_ticks',
     $h1o$      for cu in select * from combat_units where encounter_id = e.id and main_ship_id is not null and alive_count > 0 loop
        perform mainship_mark_legacy_in_flight(cu.main_ship_id, 'returning');
      end loop;$h1o$,
     $h1n$      for cu in select * from combat_units where encounter_id = e.id and main_ship_id is not null loop
...
begin
  ...
  select regexp_replace(p.prosrc, '--[^' || chr(10) || ']*', '', 'g') into v_hp
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
