...
begin
  for r in
    ...
    (1, 'command_ship_group_go',
     $h1o$  -- 5) members. Read-only: the members are the fleet's manifest, never movement subjects.
  ...
  if v_member_n = 0 then
    return jsonb_build_object('ok', false, 'reason', 'empty_group');
     ...
     $h1n$  -- 5) members. Read-only: the members are the fleet's manifest, never movement subjects.
  select coalesce(array_agg(main_ship_id order by created_at), '{}')
    into v_members
    from public.main_ship_instances
   where group_id = v_group and player_id = v_player;
  ...
  -- ── end 0312 — the head continues verbatim from here ───────────────────────────────────────────$h1n$),
    (2, 'command_ship_group_dock',
     $h2o$  v_group := public.mainship_resolve_owned_group(v_player, p_group_id);
  ...
  if not found then
    return jsonb_build_object('ok', false, 'reason', 'group_not_found');
     ...
     $h2n$  v_group := public.mainship_resolve_owned_group(v_player, p_group_id);
  if v_group is null then
    return jsonb_build_object('ok', false, 'reason', 'group_not_found');
  end if;
  perform 1 from public.ship_groups where group_id = v_group and player_id = v_player for update;
  ...
  -- ── end 0312 — the head continues verbatim from here ───────────────────────────────────────────$h2n$)
