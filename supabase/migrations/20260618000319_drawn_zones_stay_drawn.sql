...
begin
  for r in
    select * from (values
    (1, 'zone_update',
     $h1o$  -- rollback — no torn write). ONLY boundary + name + location_id + updated_at are written: source
  -- ('drawn'), zone_kind ('pirate'), created_by and created_at are preserved bit-for-bit (a zone edit
  -- cannot change what KIND of row this is or who authored it).$h1o$,
     $h1n$  -- rollback — no torn write). boundary + name + location_id + updated_at + revision are written, and
  ...
  -- reversibility claim 0282/0283 rest on — its immutable protection class.$h1n$),
    (2, 'zone_update',
     $h2o$    update public.danger_zones
       set boundary    = v_boundary,
           ...
           revision    = revision + 1$h2o$,
     $h2n$    update public.danger_zones
       set boundary    = v_boundary,
           name        = v_name,
           location_id = v_attach,
...
begin
  select string_agg(z.name, ', ' order by z.name) into v_names
    from public.danger_zones z
   where z.source = 'circle' and exists (
      select 1
        from public.world_editor_audit a
       where a.command_type = 'zone_update'
         and a.target_type  = 'zone'
         ...
         and public.st_orderingequals(
               z.boundary,
               public.st_geomfromtext(a.after_snapshot->>'boundary_wkt', public.st_srid(z.boundary))));
