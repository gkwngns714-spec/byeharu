...
begin
  for r in
    ...
    (1, 'combat_create_group_encounter',
     $h1o$  v_weapons_json    jsonb;$h1o$,
     $h1n$  v_weapons_json    jsonb;
  ...
  select gsm.main_ship_id into v_lead_ship_id
   ...
   order by msi.is_command_ship desc, msi.max_hp desc, gsm.main_ship_id asc
   limit 1;$h2n$),
    (3, 'combat_create_group_encounter',
     $h3o$    select gsm.main_ship_id, gsm.player_id, msi.hp, msi.shield, msi.max_shield, msi.is_command_ship$h3o$,
     $h3n$    -- 0315: is_command_ship is GONE from this projection. The flag is consulted EXACTLY ONCE in
    -- this function — in the lead derivation above — and the per-member question is v_is_lead.
    select gsm.main_ship_id, gsm.player_id, msi.hp, msi.shield, msi.max_shield$h3n$),
    (4, 'combat_create_group_encounter',
     $h4o$    v_aggro_priority := case when m.is_command_ship then 100 else 0 end;$h4o$,
     $h4n$    -- 0315: ONE authority for "who leads this formation" — the same v_is_lead decides the anchor
    ...
    v_aggro_priority := case when v_is_lead then 100 else 0 end;$h4n$),
    (5, 'combat_create_group_encounter',
     $h5o$          if m.is_command_ship then
            v_pos_x := v_loc_x;
            v_pos_y := v_loc_y;
          else
     ...
     $h5n$          -- 0315: the anchor spawn asks the SAME v_is_lead the aggro screen asked. The pairing is
