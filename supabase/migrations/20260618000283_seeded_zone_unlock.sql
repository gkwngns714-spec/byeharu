-- Byeharu — SEEDED-ZONE UNLOCK (migration 0283). Re-points the three seeded-zone guards at the
-- immutable `provenance` column and puts them behind TWO flags, both seeded FALSE.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- WHAT THIS FINALLY ANSWERS
-- The owner asked to be able to edit the three seeded zones. The obvious version of that — let the
-- editor flip source='circle' to 'drawn' — was rejected, and correctly: `source` was carrying both
-- geometry representation AND protection class, so adopting a zone made it permanently
-- indistinguishable from owner content. Turning the feature back off would not re-protect it.
--
-- 0282 split those meanings and froze `provenance` behind an immutability trigger. THAT is what makes
-- this reversible: flipping a flag off restores protection exactly, because provenance cannot have
-- moved in the meantime. The flag is a real toggle now, not a one-way door.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- WHY THIS LANDS BEHAVIOUR-NEUTRAL
-- Old test:  source <> 'drawn'                                    ⇒ protected
-- New test:  provenance = 'seeded' AND NOT <flag>                 ⇒ protected
-- 0282 proved ZERO rows disagree between `source <> 'drawn'` and `provenance = 'seeded'`, and both
-- flags are seeded false, so `NOT <flag>` is true for every call. The two tests are therefore
-- identical on every existing row on the day this deploys. The self-assert re-proves that agreement
-- here rather than trusting the earlier migration.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- TWO FLAGS, NOT ONE
-- seeded_zone_edit_enabled       — reshape/rename a seeded zone (zone_update)
-- seeded_zone_lifecycle_enabled  — remove one from, or restore one to, the live map
--                                  (zone_unpublish + zone_set_active)
-- Editing a zone's geometry and REMOVING it from the player map are different-sized decisions, and
-- one flag would mean enabling the first silently granted the second. Unpublish and set_active share
-- a flag because they are inverses of one lifecycle authority.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- HOW THESE FUNCTIONS WERE PRODUCED
-- Each body is copied VERBATIM from its own migration and altered at exactly ONE place — the
-- protection test — so the surrounding owner gating, request_id idempotency, row locking and
-- optimistic concurrency cannot drift through hand-transcription. A test asserts distinctive
-- untouched lines survive character-for-character.
--
-- NOT CHANGED HERE: the misleading error PRECEDENCE. Editing a seeded zone still reports
-- stale_revision before it reports protected_zone, because the concurrency compare runs first. That
-- reordering moves code inside a live command and deserves its own slice; this one only changes what
-- the protection test READS.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
...
begin
  if not exists (select 1 from information_schema.columns
    ...
    raise exception 'SEEDED-ZONE UNLOCK 0283: danger_zones.provenance (0282) is missing';
  end if;
  if not exists (select 1 from pg_trigger where tgname = 'danger_zones_provenance_immutable') then
    raise exception 'SEEDED-ZONE UNLOCK 0283: the provenance immutability trigger is missing — without it these flags are one-way doors';
  end if;
end $pre$;

-- ── 1. the two flags, both seeded FALSE ─────────────────────────────────────────────────────────
insert into public.game_config (key, value, description) values
  ('seeded_zone_edit_enabled', 'false'::jsonb,
   'Allow zone_update to reshape/rename a provenance=seeded zone. Seeded false. Reversible: '
   'provenance is immutable, so turning this off restores protection exactly.'),
  ('seeded_zone_lifecycle_enabled', 'false'::jsonb,
   'Allow zone_unpublish / zone_set_active on a provenance=seeded zone — i.e. removing one from, or '
   'restoring one to, the live player map. Seeded false, and SEPARATE from editing: enabling a '
   'reshape must never silently grant removal.')
on conflict (key) do nothing;

-- ── 2. the three commands, re-pointed at provenance ─────────────────────────────────────────────
-- ── from 20260618000266_worldeditor_publish_zone_update.sql: protection now reads provenance, gated by seeded_zone_edit_enabled ──
...
-- ── from 20260618000255_worldeditor_publish_zone_unpublish.sql: protection now reads provenance, gated by seeded_zone_lifecycle_enabled ──
...
-- ── from 20260618000268_worldeditor_zone_setactive.sql: protection now reads provenance, gated by seeded_zone_lifecycle_enabled ──
...
begin
  ...
  if coalesce(public.cfg_bool('seeded_zone_edit_enabled'), true) then
    raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: seeded_zone_edit_enabled is not false'; end if;
  if coalesce(public.cfg_bool('seeded_zone_lifecycle_enabled'), true) then
    raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: seeded_zone_lifecycle_enabled is not false'; end if;

  -- (2) BEHAVIOUR NEUTRALITY re-proven here, not merely inherited from 0282: with both flags dark
  -- the new test must classify every existing row exactly as the old one did.
  select count(*) into v_disagree
    from public.danger_zones
   where (source <> 'drawn') <> (provenance = 'seeded');
  if v_disagree > 0 then
    raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: % row(s) would change protection status', v_disagree;
  end if;

  -- (3) every guard now reads provenance, and NONE still reads source for protection
  foreach v_fn in array array['public.zone_update(text, jsonb)',
    ...
    if v_def is null then
      raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: % vanished', v_fn; end if;
    if strpos(v_def, 'v_live.provenance = ''seeded''') = 0 then
      raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: % does not gate on provenance', v_fn; end if;
    if strpos(v_def, 'v_live.source <> ''drawn''') > 0 then
      raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: % still gates protection on source', v_fn; end if;
    if strpos(v_def, 'protected_zone') = 0 then
      raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: % lost its protected_zone code', v_fn; end if;
    -- the surrounding house gating must have survived the re-creation intact
    if strpos(v_def, 'not_authenticated') = 0 or strpos(v_def, 'is_owner()') = 0
       or strpos(v_def, 'world_editor_audit') = 0 or strpos(v_def, 'for update') = 0 then
      raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: % lost part of its gate chain', v_fn; end if;
  end loop;

  -- (4) the two flags are genuinely separate: edit must not appear in the lifecycle commands
  if strpos(pg_get_functiondef(to_regprocedure('public.zone_unpublish(text, jsonb)')),
            'seeded_zone_edit_enabled') > 0
     or strpos(pg_get_functiondef(to_regprocedure('public.zone_set_active(text, jsonb)')),
               'seeded_zone_edit_enabled') > 0 then
    raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: a lifecycle command reads the EDIT flag';
  end if;
  if strpos(pg_get_functiondef(to_regprocedure('public.zone_update(text, jsonb)')),
            'seeded_zone_lifecycle_enabled') > 0 then
    raise exception 'SEEDED-ZONE UNLOCK 0283 self-assert FAIL: zone_update reads the LIFECYCLE flag';
  end if;

  raise notice 'SEEDED-ZONE UNLOCK 0283 self-assert ok: both flags exist and are FALSE, so protection is byte-identical today — re-proven by finding ZERO rows where (source <> ''drawn'') disagrees with (provenance = ''seeded''); all three commands now gate on the IMMUTABLE provenance and none still gates protection on source, so turning a flag off restores protection exactly rather than leaving an adopted zone permanently unprotected; the edit and lifecycle flags are genuinely separate; every command kept its full house gate chain';
end $unlock$;
