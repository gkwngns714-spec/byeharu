-- Byeharu — E5: RESOLVER PER-ENCOUNTER VARIETY + ZERO-ELITE READINESS (migration 0261). Resolver-only,
-- DARK, flag-gated behind the SAME quad-flag as E3 (encounter_resolver_enabled seeded false). It does two
-- things, both inert while the resolver is off:
--   (1) VARIETY — resolve_location_encounter gains a per-encounter seed p_seed folded into every sampling
--       salt. process_combat_ticks passes e.id::text (an already-populated existing combat_encounters id),
--       so two encounters at the SAME location can resolve DIFFERENT (but still fully deterministic)
--       compositions. Today E3 salted location-static (a location always rolled the same shape); E5 makes
--       the roll per-encounter-varied while preserving the 0041 determinism law (hashtextextended over
--       ':enc:' salts only — NO random()/setseed).
--   (2) ZERO-ELITE READINESS — the resolver DROPS the inert is_elite roll entirely (elite produces no
--       combat effect until the elite stat-wiring slice). No is_elite is persisted (persisting is_elite:false
--       would imply a real mechanic). Instead the returned plan carries a single honest top-level marker
--       'elite_policy':'disabled_v1'. The activate-encounter-resolver act gains a readiness guard that
--       REFUSES to flip while any active binding reaches a fleet member with elite_chance>0.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- THE HARD GUARANTEE (same as E3): with encounter_resolver_enabled=false (default) the resolved branch is
-- UNREACHABLE and combat is BYTE-IDENTICAL to today. process_combat_ticks is re-emitted CHARACTER-FOR-
-- CHARACTER from 0260 except ONE line — the resolver call now passes the seed (e.id::text). That line lives
-- inside `if v_resolver_engaged ... if e.resolved_plan_json is null`, unreachable when the flag is off.
-- Re-proven by scripts/encounter-resolver-proof (all existing markers + the new E5 markers) and this
-- migration's own self-assert.
--
-- STACKS ON: E3 (0260) — replaces resolve_location_encounter's signature (uuid -> uuid,text) and re-creates
-- process_combat_ticks. Out of scope: elite stat wiring (a later slice), activation (a human gate).
-- ═══════════════════════════════════════════════════════════════════════════════════════════════

-- ── 0. dependency gate — abort loudly if an E3 surface this slice rebuilds is missing ─────────────
do $e5dep$
begin
  if to_regprocedure('public.resolve_location_encounter(uuid)') is null then
    raise exception 'E5-VARIETY: resolve_location_encounter(uuid) (0260/E3) missing — this slice replaces its signature';
  end if;
  if to_regprocedure('public.process_combat_ticks()') is null then
    raise exception 'E5-VARIETY: process_combat_ticks() (0260/E3) missing — this slice re-creates it';
  end if;
  if to_regclass('public.encounter_runtime_state') is null then
    raise exception 'E5-VARIETY: encounter_runtime_state (0260/E3) missing';
  end if;
  if not exists (select 1 from information_schema.columns
      where table_schema='public' and table_name='combat_encounters' and column_name='resolved_plan_json') then
    raise exception 'E5-VARIETY: combat_encounters.resolved_plan_json (0260/E3) missing';
  end if;
end $e5dep$;

-- ── 1. resolve_location_encounter — the SIGNATURE CHANGE (adds the per-encounter seed) + ZERO-ELITE ─
-- Drop the E3 single-arg form (the blast radius: every activation-script/proof/doc pin moves to (uuid,text)
-- in lockstep), then recreate with p_seed folded into each ':enc:' sampling salt. The is_elite roll is
-- REMOVED (elite deferred until the elite stat-wiring slice); the plan carries elite_policy=disabled_v1.
-- Everything else — the quad-flag gate, location-active check, cap/cooldown, weight walks, total-clamp,
-- skip-zero, shared-reward resolution, plan shape — is byte-identical to 0260. Determinism preserved; STABLE.
drop function if exists public.resolve_location_encounter(uuid);
...
begin
  ...
  --     E5 (0261): elite deferred until the elite stat-wiring slice — NO elite roll; the plan carries elite_policy=disabled_v1.
...
comment on function public.resolve_location_encounter(uuid, text) is
  'ENCOUNTER RESOLVER (0261/E5): quad-flag-gated deterministic encounter planner with a PER-ENCOUNTER seed. '
  'Returns ONE resolved plan {encounter_profile_id, active_encounter_cap, cooldown_seconds, '
  'elite_policy:disabled_v1, reward_profile:{id,resource_grants}, units:[{enemy_archetype_id,unit_type_id,'
  'base_difficulty,count,stat_overrides}]} or NULL. p_seed folds into every :enc: sampling salt so two '
  'encounters at one location vary while staying deterministic. hashtextextended over '':enc:'' salts only — '
  'NO random()/setseed (the 0041 law). Elite deferred until the elite stat-wiring slice; plan tagged '
  'elite_policy=disabled_v1 (no is_elite persisted).';
...
-- ── 2. process_combat_ticks — re-emitted CHARACTER-FOR-CHARACTER from 0260 except the ONE resolver
--    call line, which now passes the per-encounter seed e.id::text (e is the existing combat_encounters
--    row already being iterated, so e.id is populated). The changed line is inside the resolver-engaged /
--    first-resolve branch — unreachable when the flag is off ⇒ combat byte-identical. ─────────────────
...
begin
  ...
  if v_tick is null or v_res is null then
    raise exception 'E5-VARIETY self-assert FAIL: process_combat_ticks / resolve_location_encounter missing';
  end if;

  -- (1) the flag is (still) committed DARK.
  if coalesce((select value #>> '{}' from public.game_config where key = 'encounter_resolver_enabled'), 'false') <> 'false' then
    raise exception 'E5-VARIETY self-assert FAIL: encounter_resolver_enabled is not seeded false';
  end if;

  -- (2) THE VERBATIM FLAG-OFF ARMS survive in process_combat_ticks — the byte-identity anchors.
  if strpos(v_tick, 'v_enemy_count  := least(coalesce(cfg_num(''enemy_synthetic_max_units''),6)::integer, greatest(1, v_danger));') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: the verbatim synthetic-wave count line is gone (byte-identity breach)'; end if;
  if strpos(v_tick, '''module_type_id'', ''pirate_synthetic_weapon'', ''range'', v_enemy_range,') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: the verbatim synthetic-wave weapon line is gone (byte-identity breach)'; end if;
  if strpos(v_tick, 'encounter_id, player_id, unit_type_id, side, ship_hp, initial_count, alive_count,') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: the verbatim synthetic-wave insert is gone (byte-identity breach)'; end if;
  ...
  if strpos(v_tick, 'v_reward_metal := round(coalesce(cfg_num(''reward_metal_base''),10) * greatest(loc.reward_tier,1)') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: the verbatim :898 reward formula head is gone (byte-identity breach)'; end if;
  if strpos(v_tick, '* (1 + coalesce(cfg_num(''reward_danger_scale''),0.25) * v_danger) * coalesce(cfg_num(''reward_multiplier''),1.0));') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: the verbatim :899 reward formula tail is gone (byte-identity breach)'; end if;
  ...
  if strpos(v_tick, 'coalesce(sum(coalesce(cu2.attack_snapshot, ut.attack) * cu2.alive_count), 0)') = 0
    ...
    raise exception 'E5-VARIETY self-assert FAIL: the 0234 aggregate (C) arm is altered (out-of-scope change)'; end if;

  -- (3) the NEW SEEDED resolver call is wired in the tick; the resolved branch + reward adapter present.
  if strpos(v_tick, 'v_resolver_engaged') = 0
     or strpos(v_tick, 'resolve_location_encounter(e.location_id, e.id::text)') = 0
     or strpos(v_tick, 'resolve_encounter_reward_inputs(') = 0
     or strpos(v_tick, 'encounter_runtime_state') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: the SEEDED resolved call / reward adapter / runtime-state wiring is missing'; end if;

  -- (4) determinism: NO new random() in the tick beyond the 0234 head's two per-arm variance rolls; the
  ...
  if v_n <> 2 then
    raise exception 'E5-VARIETY self-assert FAIL: process_combat_ticks carries % random( call(s) (want exactly 2 — E5 adds none)', v_n; end if;
  if strpos(v_res, 'random(') > 0 or strpos(v_res, 'setseed') > 0 then
    raise exception 'E5-VARIETY self-assert FAIL: resolve_location_encounter carries a session-RNG token (the 0041 determinism law)'; end if;
  if strpos(v_res, 'hashtextextended') = 0 or strpos(v_res, ':enc:') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: resolve_location_encounter lost the hashtextextended / '':enc:'' deterministic-roll technique'; end if;
  -- the per-encounter seed is folded in (p_seed present in the resolver body).
  if strpos(v_res, 'p_seed') = 0 then
    raise exception 'E5-VARIETY self-assert FAIL: resolve_location_encounter does not fold in the per-encounter seed (p_seed)'; end if;

  -- (5) ZERO-ELITE: the resolver carries NO is_elite and NO ':enc:elite' salt; it DOES tag elite_policy.
  if v_res ilike '%is_elite%' then
    raise exception 'E5-VARIETY self-assert FAIL: resolve_location_encounter still references is_elite (zero-elite breach)'; end if;
  if v_res ilike '%:enc:elite%' then
    raise exception 'E5-VARIETY self-assert FAIL: resolve_location_encounter still carries the :enc:elite salt (zero-elite breach)'; end if;
  if v_res not ilike '%disabled_v1%' then
    raise exception 'E5-VARIETY self-assert FAIL: resolved plan does not carry elite_policy=disabled_v1 (the honest zero-elite signal)'; end if;

  -- (6) the out-of-scope engine functions are NOT redefined here — they exist and carry NO E3/E5 token.
  if v_ccge is null or v_report is null or v_reward is null or v_base is null then
    raise exception 'E5-VARIETY self-assert FAIL: an out-of-scope engine function is missing'; end if;
  if strpos(v_ccge, 'resolve_location_encounter') > 0 or strpos(v_ccge, 'resolved_plan_json') > 0
    ...
    raise exception 'E5-VARIETY self-assert FAIL: an out-of-scope engine function carries an E3/E5 token (blast radius breach)'; end if;

  -- (7) ACL: the tick + the seeded resolver stay non-client-executable (engine-only).
  if has_function_privilege('authenticated', 'public.process_combat_ticks()', 'execute')
     ...
     or has_function_privilege('authenticated', 'public.resolve_location_encounter(uuid,text)', 'execute')
     or has_function_privilege('anon', 'public.resolve_location_encounter(uuid,text)', 'execute') then
    raise exception 'E5-VARIETY self-assert FAIL: an engine function is client-executable'; end if;

  raise notice 'E5-VARIETY self-assert ok: encounter_resolver_enabled still seeded dark; process_combat_ticks re-emitted byte-identical except the one seeded resolver call (resolve_location_encounter(e.location_id, e.id::text)); the verbatim 0234 synthetic-wave + :898-899 reward lines + the whole aggregate (C) arm survive; the tick carries exactly 2 random( calls (E5 adds none); resolve_location_encounter folds in p_seed, is hashtextextended/'':enc:''-deterministic with no RNG, carries NO is_elite / no :enc:elite salt and tags elite_policy=disabled_v1; combat_create_group_encounter/report_create/reward_grant/base_add_resources are not redefined and carry no E3/E5 token; ACL on the engine functions unchanged (non-client-executable)';
end $e5assert$;
