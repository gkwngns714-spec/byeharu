...
begin
  for r in
    ...
    (1, 'process_combat_ticks',
     $h1o$  v_ring_radius            double precision;
  v_spawn_slot             integer;$h1o$,
     $h1n$  v_ring_radius            double precision;
  ...
  v_pirate_danger          double precision;$h1n$),
    (2, 'process_combat_ticks',
     $h2o$          v_enemy_hp     := loc.base_difficulty * coalesce(cfg_num('enemy_hp_base'),14)
          ...
          v_enemy_count  := least(coalesce(cfg_num('enemy_synthetic_max_units'),6)::integer, greatest(1, v_danger));$h2o$,
     $h2n$          -- ██ 0341 THE OWNER: "round 1~5 should be only 1, then round 5~10 should be 2" ██
