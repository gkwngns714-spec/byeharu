-- Byeharu — WORLD EDITOR PUBLISHING SLICE 3: exploration_site_update — the FIRST live-world-UPDATE
-- publish command, owner-gated through the 0243 contract, with OPTIMISTIC CONCURRENCY.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- WHAT THIS IS: the first World Editor command that EDITS existing world content — it UPDATEs one
-- row in public.exploration_sites (the row an EDIT draft was forked from). It reproduces the 0244
-- exploration_site_create template step for step ((1) authn → typed 'not_authenticated'; (2) authz
...
-- details[]; typed 'conflict' on the unique name) and adds the parts an UPDATE needs:
--
--   • TARGETING: p_payload.target_id is the site's CURRENT name — the natural key the edit draft
--     forked from (the editor's read contract exposes no client-visible uuid; see
--     explorationDraftModel.ts liveId). The live row is located AND ROW-LOCKED (select … for
...
--   • reward_bundle_json is UNOBSERVABLE client-side (0098: RLS server-only; the editor's read is
--     name + coords only — explorationDraftTypes.ts), so an edit fork honestly carries null for it.
...
--     the write) and after_snapshot (the returned row after) — the 0244 columns, now both used.
--
...
-- this SECURITY DEFINER function only; the 0244 write-grant revocation is re-asserted (never
-- re-granted) below.
--
...
-- authority), no reuse of the gameplay RPC surface (command_exploration_scan/securing untouched).
-- world_editor_ping / exploration_site_create / mining_field_create are left byte-identical; the
-- 0244 audit snapshot columns are NOT re-added (the dependency gate asserts they exist).
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
...
begin
  if to_regclass('public.exploration_sites') is null then
    raise exception 'PUBLISH-EXPL-UPDATE: public.exploration_sites (0098) is missing — nothing to update';
  end if;
  if to_regclass('public.world_editor_audit') is null then
    raise exception 'PUBLISH-EXPL-UPDATE: public.world_editor_audit (0243) is missing — the audit/idempotency spine must exist first';
  end if;
  if to_regprocedure('public.is_owner()') is null then
    raise exception 'PUBLISH-EXPL-UPDATE: public.is_owner() (0243) is missing — THE ONE owner guard must exist first';
  end if;
  if to_regprocedure('public.exploration_site_create(text, jsonb)') is null then
    raise exception 'PUBLISH-EXPL-UPDATE: public.exploration_site_create (0244) is missing — the publish template this extends must exist';
  end if;
  ...
  if not exists (select 1 from pg_attribute
    ...
    raise exception 'PUBLISH-EXPL-UPDATE: a 0244 audit snapshot column (before_snapshot/after_snapshot) is missing — the snapshot columns must exist first';
  end if;
  if to_regprocedure('public.pirate_zone_create(text, jsonb, uuid)') is null
    ...
    raise exception 'PUBLISH-EXPL-UPDATE: a 0239 pirate_zone write RPC is missing — the lockdown surface must exist to be re-asserted';
  end if;
end $pubdep$;

-- ── 1. exploration_site_update — the FIRST live-world-UPDATE command (0244 template + concurrency) ─
-- TYPED RESULT/ERROR CONTRACT (extends the 0244 vocabulary):
...
--             , 'conflict'           -- the NEW name collides with another site's unique name
--             }
-- p_payload = { target_id:      <the site's CURRENT name — the natural key the draft forked from>,
...
begin
  ...
  -- rules as exploration_site_create (the client's advisory validator is NEVER trusted; every
  ...
  -- cannot author the bundle it cannot read — explorationDraftTypes.ts). Name UNIQUENESS is
  ...
  -- constraint's table disambiguates: exploration_sites.name (a RENAME collided with another
  -- site) ⇒ typed 'conflict'; world_editor_audit.request_id ⇒ a concurrent duplicate raced us ⇒
...
comment on function public.exploration_site_update(text, jsonb) is
  'WORLD EDITOR PUBLISH SLICE (0247): the FIRST live-world-UPDATE command — edits ONE exploration '
  'site from an owner EDIT draft with OPTIMISTIC CONCURRENCY. Reproduces the 0243/0244 template: '
  'authn → is_owner() authz → request_id idempotency → one audit row → typed envelope; then locates '
  '+ ROW-LOCKS the target by its natural key (payload.target_id = the current name; not_found/'
  'source_missing when gone), rejects any fork-time drift with stale_revision/source_changed '
  '(value-by-value against payload.expected — the client fingerprint is never re-derived), '
  're-validates the new fields server-side (validation_failed + details[]; a null '
  ...
  'exploration_sites write happens only inside this SECURITY DEFINER body — no client grant.';
...
-- NOTE: the exploration_sites client write grants were already revoked by 0244 (a pure narrowing).
...
begin
  ...
  if to_regclass('public.app_owners') is null then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: app_owners missing';
  end if;
  if to_regprocedure('public.is_owner()') is null then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: is_owner() missing';
  end if;
  if to_regclass('public.world_editor_audit') is null then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: world_editor_audit missing';
  end if;
  ...
  if not exists (select 1 from pg_attribute
    ...
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: an audit snapshot column (before_snapshot/after_snapshot/source_revision) is missing';
  end if;
  ...
  if to_regprocedure('public.exploration_site_update(text, jsonb)') is null then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: exploration_site_update(text,jsonb) missing';
  end if;
  if not exists (select 1 from pg_proc
                 where oid = 'public.exploration_site_update(text,jsonb)'::regprocedure and prosecdef) then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: exploration_site_update is not SECURITY DEFINER';
  end if;
  if not has_function_privilege('authenticated', 'public.exploration_site_update(text,jsonb)', 'execute') then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: authenticated cannot execute exploration_site_update — the in-body guard would be unreachable';
  end if;
  if has_function_privilege('anon', 'public.exploration_site_update(text,jsonb)', 'execute') then
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: anon CAN execute exploration_site_update — must be authenticated-only';
  end if;

  -- (d) NO client-role write grant on exploration_sites — the 0244 revocation is INTACT (this
  ...
  if has_table_privilege('authenticated', 'public.exploration_sites', 'INSERT')
    ...
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: a client role holds an INSERT/UPDATE grant on exploration_sites — the only write path must be the SECURITY DEFINER command';
  end if;
  ...
  if has_function_privilege('authenticated', 'public.pirate_zone_create(text,jsonb,uuid)', 'execute')
    ...
    raise exception 'PUBLISH-EXPL-UPDATE self-assert FAIL: a client role regained EXECUTE on a pirate_zone write RPC — the 0239 lockdown was disturbed';
  end if;

  raise notice 'PUBLISH-EXPL-UPDATE self-assert ok: 0243 spine + 0244 snapshot columns present; exploration_site_update SECURITY DEFINER + authenticated-only; no client write grant on exploration_sites; 0239 lockdown intact';
end $pubassert$;
