...
create temporary table _v1c0253_pre_locations on commit drop as
  select id, x, y, territory_radius from public.locations;
create temporary table _v1c0253_pre_sectors on commit drop as
  select id, x, y from public.sectors;
create temporary table _v1c0253_pre_zones on commit drop as
  select id, x, y, radius from public.zones;
create temporary table _v1c0253_pre_danger on commit drop as
  select id, boundary from public.danger_zones;
create temporary table _v1c0253_pre_moves on commit drop as
  select id, origin_type, origin_x, origin_y, target_type, target_x, target_y
    from public.fleet_movements where status = 'moving';
create temporary table _v1c0253_pre_mining on commit drop as
  select id, space_x, space_y from public.mining_fields;
create temporary table _v1c0253_pre_exploration on commit drop as
  select id, space_x, space_y from public.exploration_sites;

create temporary table _v1c0253_scale (k integer not null) on commit drop;
...
begin
  ...
  insert into _v1c0253_scale (k) values (v_k);
...
update public.locations
   set x = x * (select k from _v1c0253_scale),
       y = y * (select k from _v1c0253_scale),
       territory_radius = territory_radius * (select k from _v1c0253_scale);
...
update public.zones
   set x = x * (select k from _v1c0253_scale),
       y = y * (select k from _v1c0253_scale),
       radius = radius * (select k from _v1c0253_scale);
...
update public.danger_zones
   set boundary = ST_Scale(boundary,
                           (select k from _v1c0253_scale)::double precision,
                           (select k from _v1c0253_scale)::double precision),
       updated_at = now();
...
update public.fleet_movements
   set target_x = target_x * (select k from _v1c0253_scale),
       target_y = target_y * (select k from _v1c0253_scale)
 where status = 'moving' and target_type in ('location', 'zone');

update public.fleet_movements
   set origin_x = origin_x * (select k from _v1c0253_scale),
       origin_y = origin_y * (select k from _v1c0253_scale)
...
begin
  ...
  -- The 0227 body verbatim, with the two coordinate arrays as placeholders (0253 HUNK) and the HUNK
...
declare
  ...
  -- 0253 HUNK 1: the approved anchor coordinate is the V1C-normalized (×k) value, injected from the
...
begin
  ...
  -- 0253 HUNK 2 (unchanged from 0227): lock whichever location-kind anchor is CURRENTLY active for
  ...
  for i in 1..3 loop
    ...
    -- 0253 HUNK 2 (cont., unchanged from 0227): exactly one active canonical anchor owned by THIS
...
declare
  ...
  v_ay_txt     text;
begin
  select k into v_k from _v1c0253_scale;
  ...
  if v_locations = 0 then raise exception '0253 self-assert FAIL: no locations — every sweep vacuous'; end if;
  select count(*) into v_n from public.locations where territory_radius is not null;
  if v_n = 0 then raise exception '0253 self-assert FAIL: no territory-bearing locations — the disjointness sweep would be vacuous'; end if;
  select count(*) into v_danger from public.danger_zones where source = 'circle';
  if v_danger = 0 then raise exception '0253 self-assert FAIL: no source=circle danger zones — the containment sweep would be vacuous (0233 did not seed)'; end if;
  ...
  if v_anchors <> v_locations then
    raise exception '0253 self-assert FAIL (A): % active location anchor(s) for % location(s)', v_anchors, v_locations;
  end if;
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (A): % active anchor(s) differ from their location''s exact post-scale (x,y)', v_n;
  end if;
  select count(*) into v_n
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (A): % location(s) with more than one active anchor', v_n;
  end if;
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (B): % location(s)/ring(s) outside the ±10000 envelope', v_n; end if;
  select count(*) into v_n from public.sectors where greatest(abs(x), abs(y)) > 10000;
  if v_n <> 0 then raise exception '0253 self-assert FAIL (B): % sector(s) outside the envelope', v_n; end if;
  select count(*) into v_n from public.zones
   where greatest(abs(x), abs(y), abs(x) + radius, abs(y) + radius) > 10000;
  if v_n <> 0 then raise exception '0253 self-assert FAIL (B): % zone(s) outside the envelope', v_n; end if;
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (B): % danger zone(s) outside the envelope', v_n; end if;
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (B): % anchor(s) outside the envelope', v_n; end if;
  ...
  select count(*) into v_n
    ...
    join _v1c0253_pre_locations pa on pa.id = a.id
    join _v1c0253_pre_locations pb on pb.id = b.id
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (C): % location pair(s) broke distance-×k parity vs the snapshot pre-image', v_n;
  end if;
  select count(*) into v_n
    from public.locations a
    join public.locations b on a.id < b.id
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (C): % location pair(s) changed bearing — the scale is not uniform', v_n;
  end if;
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (D1): % pair(s) flipped overlap status across the scale', v_n;
  end if;
  select count(*) into v_n
    from public.locations a
    join public.locations b on a.id < b.id
   where a.territory_radius is not null and b.territory_radius is not null
     and public.osn_distance(a.x, a.y, b.x, b.y) <= (a.territory_radius + b.territory_radius);
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (D2): % overlapping territory pair(s) on the post-scale world', v_n;
  end if;
  select count(*) into v_n
    from public.locations a
    join public.locations b on a.id <> b.id
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (D2): % ring(s) reach another location''s center on the post-scale world', v_n;
  end if;
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (E): % circle danger zone(s) no longer contain their location''s center', v_n;
  end if;
  ...
  select count(*) into v_n
    ...
    join _v1c0253_pre_moves p on p.id = m.id
  ...
  if v_n <> 0 then
    raise exception '0253 self-assert FAIL (F): % in-flight leg(s) off their exact ×k (location/zone) or untouched (space/base) pre-image', v_n;
  end if;
  ...
  if v_ax_txt is null or v_ay_txt is null then
    raise exception '0253 self-assert FAIL (G): a starter-port row is missing — the array pin cannot be built';
  end if;
  select prosrc into v_src from pg_proc where oid = to_regprocedure('public.reveal_starter_ports()')::oid;
  if v_src is null then
    raise exception '0253 self-assert FAIL (G): public.reveal_starter_ports() does not exist';
  end if;
  if position('array[' || v_ax_txt || ']' in v_src) = 0 or position('array[' || v_ay_txt || ']' in v_src) = 0 then
    raise exception '0253 self-assert FAIL (G): reveal_starter_ports() does not carry the normalized (×k) anchor coordinate arrays [%] / [%]', v_ax_txt, v_ay_txt;
  end if;
  if v_k > 1 and (position('array[-150, 210, 30]' in v_src) > 0 or position('array[-90, -30, 240]' in v_src) > 0) then
    raise exception '0253 self-assert FAIL (G): reveal_starter_ports() still carries a pre-normalize coordinate array — the re-create did not land';
  end if;
  if position('a.id = v_anchors[i]' in v_src) > 0 then
    raise exception '0253 self-assert FAIL (G): reveal_starter_ports() pins a fixed anchor-id literal — it cannot survive retire+insert relocation';
  end if;
  ...
  if v_src is null then
    raise exception '0253 self-assert FAIL (H): public.get_world_map() does not exist';
  end if;
  if position('l.zone_id = z.id and l.status = ''active''' in v_src) = 0
    ...
    raise exception '0253 self-assert FAIL (H): get_world_map() body drifted from the 0217 head — this slice must not touch it';
  end if;
  if position('space_anchors' in v_src) > 0 then
    raise exception '0253 self-assert FAIL (H): get_world_map() reads space_anchors — the cutover is a LATER slice (PR-D), not this one';
  end if;
  ...
  select count(*) into v_n
    ...
    full join _v1c0253_pre_mining p on p.id = f.id
   where f.id is null or p.id is null
      or f.space_x is distinct from p.space_x or f.space_y is distinct from p.space_y;
  if v_n <> 0 then raise exception '0253 self-assert FAIL (I): mining_fields changed — out of scope, must be untouched'; end if;
  select count(*) into v_n
    from public.exploration_sites e
    full join _v1c0253_pre_exploration p on p.id = e.id
   where e.id is null or p.id is null
      or e.space_x is distinct from p.space_x or e.space_y is distinct from p.space_y;
  if v_n <> 0 then raise exception '0253 self-assert FAIL (I): exploration_sites changed — out of scope, must be untouched'; end if;
  ...
  select count(*) into v_n
    from public.locations l join _v1c0253_pre_locations p on p.id = l.id
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (R): % location(s) failed the exact ×k / ÷k round trip', v_n; end if;
  select count(*) into v_n
    from public.sectors s join _v1c0253_pre_sectors p on p.id = s.id
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (R): % sector(s) failed the exact round trip', v_n; end if;
  select count(*) into v_n
    from public.zones z join _v1c0253_pre_zones p on p.id = z.id
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (R): % zone(s) failed the exact round trip', v_n; end if;
  ...
  select count(*) into v_n
    from public.danger_zones dz
    join _v1c0253_pre_danger p on p.id = dz.id
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (R): % danger zone(s) differ from ST_Scale(pre, k, k)', v_n; end if;
  select count(*) into v_n
    from public.danger_zones dz
    join _v1c0253_pre_danger p on p.id = dz.id,
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (R): % danger-zone vertex(es) failed the ÷k round trip beyond 1e-9', v_n; end if;
  -- in-flight scalar round trip for the scaled endpoints.
  select count(*) into v_n
    from public.fleet_movements m join _v1c0253_pre_moves p on p.id = m.id
   where m.status = 'moving'
  ...
  if v_n <> 0 then raise exception '0253 self-assert FAIL (R): % in-flight leg(s) failed the exact ÷k round trip', v_n; end if;

  select count(*) into v_n2 from public.danger_zones;
  raise notice '0253 self-assert ok: k=% — % location(s) + anchors (anchor==location exact, no dup-actives), % sector(s)/% zone(s) scaled, % danger zone(s) ST_Scaled (circle-center containment intact), % in-flight leg(s) reconciled, distances ×k + bearings preserved pair-wide, territory overlap-signs preserved + disjoint, reveal_starter_ports re-created ×k, get_world_map body untouched, mining/exploration untouched, EXACT ÷k reversibility proven',
    v_k, v_locations,
