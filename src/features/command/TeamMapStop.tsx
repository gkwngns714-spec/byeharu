...
import { stopShipGroup, type TeamRpcResult } from './teamApi'
import type { GroupRow } from './teamRoster'
import { groupStopAvailability, resolveStoppableFleets, stopOutcomeMessage } from './teamStop'
  ...
  const run = async (fleetName: string, key: string, op: () => Promise<TeamRpcResult>) => {
    ...
    try {
      ...
      if (!res.ok) setNotice({ tone: 'warning', text: teamReasonMessage(res.reason) })
        ...
        // 0164 is BEST-EFFORT: ok:true carries the server's own {stopped, skipped, failed} aggregate.
        // The summary reports those numbers rather than assuming every member halted.
        setNotice({ tone: 'success', text: stopOutcomeMessage(fleetName, res) })
          ...
          return (
            <div key={f.groupId} className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                onClick={() => void run(f.name, `stop:${f.groupId}`, () => stopShipGroup(f.groupId))}
              >
                Stop — hold here
              </Button>
            </div>
