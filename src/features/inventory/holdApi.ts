...
/** The player's hold: carried stacks + the server's own used/capacity/free numbers.
 *  Error / signed out / malformed → HOLD_EMPTY (fail-closed; nothing renders, nothing invented). */
export async function fetchMyHold(): Promise<Hold> {
  const { data, error } = await supabase.rpc('get_my_hold')
