...
// send_ship_group_expedition (0163), send_ship_group_hunt (0168), and the preview/totals reads
// (0165/0166) — yields specific player-facing text; any unmapped/unknown reason (incl. the teamApi
// transport 'unavailable' fallback) hits the generic "Team order unavailable." — never a raw code.
...
test('every known team-RPC reject reason maps to specific player text (not the fallback, never the raw code)', () => {
  const known = [
    // shared prefix (0163/0165/0166/0168)
