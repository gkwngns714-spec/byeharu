...
const P1 = 'b1a00001-0066-4a00-8a00-000000000001' // Haven Reach (current dock — must be EXCLUDED)
const P2 = 'b1a00002-0066-4a00-8a00-000000000002' // Slagworks Anchorage (eligible)
const P3 = 'b1a00003-0066-4a00-8a00-000000000003' // Driftmarch Waypost (eligible)
...
// The anchored post-reveal readiness the server would return for a ship docked at Haven Reach.
const anchoredState = {
  readiness: { osnAvailable: true, originCategory: 'anchored', reason: 'ok', eligibleDestinationIds: [P2, P3] },
  visibleLocations: [loc(P1, 'Haven Reach'), loc(P2, 'Slagworks Anchorage'), loc(P3, 'Driftmarch Waypost')],
...
test('rendered OSN PortNav journey: visible→exclude current→select→dispatch→in-transit→refresh→dark', async ({ page }) => {
  ...
  // 2) the current port (Haven Reach) is EXCLUDED; the active eligible ports are shown
  ...
  await expect(page.getByTestId('port-nav-travel')).toContainText('Slagworks Anchorage')
