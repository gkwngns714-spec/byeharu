...
// gate stays live for 0337 and everything after it.
{
  ...
  // 0346 IS EXEMPTED BY NAME, on the same structural footing as 0339: all three of its tick hunks
  // land in THIS MIGRATION'S OWN EMITTED TEXT — the v_ring_radius / v_spawn_slot declarations of
  // hunk 1, the v_ring_radius one-read line, and the per-unit combat_unit_decide_move call whose
  // `my_min_range` argument this file introduced. None of that is 0299 text, so no slice this file
  // takes from the 0299 head can overlap it. 0346 moves an enemy body's ORIGIN to the zone's own
  // city and gives it a bounded closing speed while it is still outside the fight; it moves no
  // radius that this file set — 0336's (measured extent + range + 1) survives verbatim as 0346's
  // clearance FLOOR — and no phase, no knob of this file's, no guard and no branch. The slices below
...
${indent}-- AND THE RADIUS IS THE PLAYER RING PLUS THE WAVE'S OWN REACH, WHICH IS THE WHOLE POINT.
${indent}-- Spawning the wave ON the escort ring would NOT establish the invariant: the escorts sit on
${indent}-- that same circle, so the nearest escort is a CHORD away — 2*R*sin(pi/16), i.e. 0.39*R — and
${indent}-- at live numbers that is 2.73 against a reach of 6.0, deep inside the wave's own range. The
${indent}-- invariant would then hold only for the lead, the one hull standing at the anchor.
${indent}-- Every player ship sits at radius <= v_ring_radius from the anchor, so putting the wave at
${indent}-- v_ring_radius + its own range + 1 makes the minimum distance from ANY player ship to ANY
${indent}-- enemy exactly that range + 1: strictly outside the wave's reach, BY CONSTRUCTION, at every
${indent}-- difficulty. That is why this slice raises no knob and asserts no knob comparison — the
${indent}-- geometry is not a tuning that can drift, it is an identity. What a CI assertion still has to
${indent}-- check is the OTHER half, which geometry cannot fix: that when the closing wave stops closing
${indent}-- it is inside the PLAYER's range (see the range-invariant assert).`;
