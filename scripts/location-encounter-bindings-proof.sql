...
begin
  ...
  if exists (select 1 from public.game_config where key = 'enemy_synthetic' || '_max_units') then
    raise exception 'LEB PROOF FAIL: the synthetic unit-cap game_config row EXISTS — 0344 deletes it (its only reader left is resolve_location_encounter, which falls back to its own constant), so something has re-authored a knob that gates nothing';
  end if;
