#!/usr/bin/env node
// gen-0307-loot-secures-on-arrival.mjs — emit (or --check) migration 0307.
//
// WHY A GENERATOR: 0307 rewrites a hunk inside movement_settle_arrival (a LIVE, cron-driven
// function) and a comment hunk inside command_ship_group_go. The 0303 lesson — "never retype a
// live function body" — cost a silently-dropped guard once already. So every `old` below is
// SLICED verbatim out of the migration that currently defines the hunk (0208 / 0301); nothing is
// retyped by hand. The emitted migration proves each slice is still what is deployed (occurs
// exactly once in pg_get_functiondef) before replacing it, and proves the body length moved by
// exactly the hunk delta — byte parity outside the hunk is a property of the method.
//
//   node scripts/gen-0307-loot-secures-on-arrival.mjs          # write the migration
//   node scripts/gen-0307-loot-secures-on-arrival.mjs --check  # fail if the file on disk drifted

import { readFileSync, writeFileSync } from 'node:fs'

const MIG = new URL('../supabase/migrations/', import.meta.url)

// LINE ENDINGS ARE PART OF THE CONTRACT (the gen-0306 lesson): the hunks are matched against
// pg_get_functiondef, whose text comes from the migration as APPLIED — always LF. A Windows
// checkout can hand this script CRLF; normalise on read and refuse to emit anything carrying a CR.
const src = (f) => readFileSync(new URL(f, MIG), 'utf8').replace(/\r\n/g, '\n').split('\n')

const F208 = '20260618000208_fleetgo_coordinate_targets.sql'
...
function slice(file, from, to, startsWith, endsWith) {
...
// ── Site 0: movement_settle_arrival's 'space' arm (0208:158-166 — the whole arm incl. its banner).
const SETTLE_OLD = slice(F208, 158, 166, 'THE ONLY DELTA vs the 0153 head', "'outcome', 'in_space', 'movement_id', m.id);")
...
// ── Site 1: command_ship_group_go's retreat-arm comment (0301:1740-1742). COMMENT-ONLY hunk: the
//    deployed prose says naming a destination FORFEITS the deposit — true before 0307, a lie after.
//    Comments in this codebase are load-bearing (bodies get copied from them), so the lie dies in
//    the same migration that makes it one. No code token changes; the length-delta assert plus the
//    comment-stripped probes in the self-asserts prove the code is byte-identical.
const MOVER_OLD = slice(F301, 1740, 1742, 'Loot earned BEFORE the order', 'Surfaced, never hidden.')
...
const SITES = [
  { fn: 'movement_settle_arrival', what: "the 'space' arrival arm deposits the carried bundle", old: SETTLE_OLD, neu: SETTLE_NEW },
  { fn: 'command_ship_group_go', what: 'the retreat-arm comment stops documenting the retired defect', old: MOVER_OLD, neu: MOVER_NEW },
]
...
for (const s of SITES) {
  for (const [k, v] of Object.entries({ old: s.old, neu: s.neu })) {
    ...
    if (v.includes('\r')) throw new Error(`site ${s.fn}.${k} carries a CR — line-ending normalisation failed`)
...
const sql = `-- 0307 — LOOT SECURES ON ARRIVAL: the 'space' settle arm deposits the reward it carries.
--
...
${rows}
  ...
  if v_done <> ${SITES.length} then
    raise exception '0307 REWRITE FAIL: rewrote % site(s), expected ${SITES.length}', v_done;
  end if;
