#!/usr/bin/env bash
# OSN-3 S4 — LOCAL REST/RPC permission proof against the disposable local Supabase PostgREST/GoTrue.
# Proves the public Data-API boundary: the S4 processor (and, defence-in-depth, the S3 writer + four S2
# helpers) are REJECTED for the anon key and for a real disposable AUTHENTICATED user JWT, so the
# cron-only processor is unreachable through client REST/RPC routes. The service-role key administers the
# throwaway user only (create/delete) — NEVER as a client-boundary substitute. Requires API_URL, ANON_KEY,
# SERVICE_ROLE_KEY.
set -uo pipefail
...
EMAIL="osn3s4rest.$(date +%s).$RANDOM@example.com"
PASS="Test123456!"
...
probe() { # $1=fn $2=bearer $3=label $4=body
  ...
  case "$body" in
    *'"ok"'*'true'*|*'movement_id'*|*'"status"'*'locked'*|*origin_kind*) echo "FAIL [$3] $1: body looks like a real result: $body"; exit 1;; esac
...
echo "OSN-3 S4 LOCAL REST/RPC PERMISSION PROOF: ALL PASSED"
