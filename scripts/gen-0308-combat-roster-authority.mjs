#!/usr/bin/env node
// gen-0308-combat-roster-authority.mjs — emit (or --check) migration 0308.
//
// WHY A GENERATOR: 0308 rewrites hunks inside FOUR live function bodies (the encounter builder, the
// ambush resolver, the flee verb and the brake). The 0303 lesson — "never retype a live function
// body" — cost a silently-dropped guard once already. So every `old_t` below is SLICED verbatim out
// of the file that last defines the hunk (0301 / 0303 / 0230 / 0305); nothing is retyped by hand.
// The migration then proves each slice is still what is deployed (occurs EXACTLY once in
// pg_get_functiondef), replaces it, and proves the length moved by exactly the hunk delta —
// byte parity outside the hunks is a property of the method, not a review promise.
//
//   node scripts/gen-0308-combat-roster-authority.mjs          # write the migration
//   node scripts/gen-0308-combat-roster-authority.mjs --check  # fail if the file on disk drifted
...
const OUT = MIG('20260618000308_combat_roster_is_live_membership.sql');
...
const F303 = load('20260618000303_intercept_manifest_count_is_a_count.sql');
const F230 = load('20260618000230_combat_telegraph.sql');
const F305 = load('20260618000305_one_sortie_authority.sql');
...
// ── The six hunks, sliced from the migrations that last define each body. ────────────────────────
// combat_create_group_encounter — head 0301:661. Its roster read (0301:726-731) and its
// fitted-weapon join (0301:773-776). Neither was touched by 0302..0307.
const B_ROSTER = slice(F301, '0301', 726, 731, 'for m in', 'order by gsm.main_ship_id');
const B_WEAPON = slice(F301, '0301', 773, 776, 'from ship_module_fittings f', 't.range is not null;');
// pirate_intercept_resolve_due_for_movement — head 0303:118. Its manifest freeze (0303:271-275)
// and the 0303 count hunk (0303:283-285).
const R_FREEZE = slice(F303, '0303', 271, 275, 'insert into public.group_sortie_members', 'on conflict (fleet_id, main_ship_id) do nothing;');
const R_COUNT = slice(F303, '0303', 283, 285, 'select count(*) into v_manifest', 'where gsm.fleet_id = v_fleet.id;');
// combat_flee_pending — head 0230. Its manifest ride-home loop (0230:215).
const F_LOOP = slice(F230, '0230', 215, 215, 'for m in select gsm.main_ship_id from group_sortie_members gsm', 'loop');
// command_ship_group_stop — the deployed body is 0301's text with 0305's rewrites applied, so the
// brake's inline release is verbatim in the EMITTED 0305 file (0305:247-248).
const S_RELEASE = slice(F305, '0305', 247, 248, 'delete from public.group_sortie_members', 'where fleet_id = v_fleet and player_id = v_player;');

// ── The replacements. One authority per concept; every rule stated exactly once. ─────────────────
const B_ROSTER_NEW = `  for m in
...
const B_WEAPON_NEW = `            from ship_module_fittings f
           ...
           where f.main_ship_id = m.main_ship_id and public.module_is_firing_weapon(t);`;

const R_FREEZE_NEW = `  -- 0308: THE FREEZE IS A SNAPSHOT, NOT AN ACCRETION. 0168 calls this table "the membership
...
const R_COUNT_NEW = `  -- 0308: measure what the builder will actually seed — the LIVE roster, through the same one
  ...
  select count(*) into v_manifest
    from public.group_sortie_live_members(v_fleet.id) gsm;`;

const F_LOOP_NEW = `  -- 0308: only ships still in the fleet's group ride home — a stale roster row must not mark a
  -- ship that long since left the group (and may be berthed at another port) as 'returning'.
  for m in select gsm.main_ship_id from public.group_sortie_live_members(v_pr.fleet_id) gsm loop`;

const S_RELEASE_NEW = `  -- 0308: the release is now THE one idiom, public.group_sortie_release — same delete, same
...
const HUNKS = [
  [1, 'combat_create_group_encounter', B_ROSTER, B_ROSTER_NEW],
  [2, 'combat_create_group_encounter', B_WEAPON, B_WEAPON_NEW],
  [3, 'pirate_intercept_resolve_due_for_movement', R_FREEZE, R_FREEZE_NEW],
  [4, 'pirate_intercept_resolve_due_for_movement', R_COUNT, R_COUNT_NEW],
  [5, 'combat_flee_pending', F_LOOP, F_LOOP_NEW],
  [6, 'command_ship_group_stop', S_RELEASE, S_RELEASE_NEW],
];
...
// Comment-stripping idiom proven by 0305/0306 against this database's settings.
...
begin
  select string_agg(p.proname, ', ') into v_bad
   ...
   where n.nspname = 'public'
     ...
     and (position('delete from public.group_sortie_members' in ${STRIP}) > 0
          or position('delete from group_sortie_members' in ${STRIP}) > 0);
...
if (sql.includes('\r')) {
  throw new Error('generated 0308 carries a CR — the rewrite hunks would never match the deployed body');
}
...
if (check) {
  ...
  try {
    ...
    console.error('0308 CHECK FAIL: migration file is missing — run the generator');
    process.exit(1);
  }
  if (onDisk.replace(/\r\n/g, '\n') !== sql) {
    console.error('0308 CHECK FAIL: the migration on disk is not what the slices generate.');
    console.error('Either a source migration drifted or the file was hand-edited. Re-run the generator.');
    process.exit(1);
  }
  console.log('0308 CHECK OK: migration matches the slices taken from 0301/0303/0230/0305.');
} else {
  writeFileSync(OUT, sql);
  console.log(`0308 written: ${OUT}`);
  console.log(`  ${HUNKS.length} hunks sliced from 0301/0303/0230/0305 (nothing retyped)`);
}
