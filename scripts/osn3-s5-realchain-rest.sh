#!/usr/bin/env bash
# OSN-3 S5 — LOCAL REST/RPC permission proof against the disposable local Supabase PostgREST/GoTrue.
# Proves the public Data-API boundary: the trusted destruction primitive dev_set_main_ship_destroyed
# (and, defence-in-depth, the S4 processor + S3 writer + four S2 helpers) are REJECTED for the anon key
# and for a real disposable AUTHENTICATED user JWT, so the destruction primitive is unreachable through
# client REST/RPC routes. The service-role key administers the throwaway user only (create/delete).
# Requires API_URL, ANON_KEY, SERVICE_ROLE_KEY.
set -uo pipefail
...
EMAIL="osn3s5rest.$(date +%s).$RANDOM@example.com"
PASS="Test123456!"
...
probe() { # $1=fn $2=bearer $3=label $4=body
  local code body
  ...
  case "$body" in
    *'"status"'*'destroyed'*|*fleets_cleaned*|*'"ok"'*'true'*|*movement_id*|*origin_kind*) echo "FAIL [$3] $1: body looks like a real result: $body"; exit 1;; esac
  echo "  ok [$3] $1: REJECTED (HTTP $code)"
}

echo "=== anon-key probes (must all be rejected) ==="
probe "$DESTROY" "$ANON_KEY" "anon" "$DBODY"
probe "$PROC" "$ANON_KEY" "anon" "$EMPTY"
probe "$WRITER" "$ANON_KEY" "anon" "$WBODY"
for h in $HELPERS; do probe "$h" "$ANON_KEY" "anon" "$HBODY"; done
echo "=== authenticated-JWT probes (must all be rejected) ==="
probe "$DESTROY" "$ACCESS" "authenticated" "$DBODY"
probe "$PROC" "$ACCESS" "authenticated" "$EMPTY"
probe "$WRITER" "$ACCESS" "authenticated" "$WBODY"
for h in $HELPERS; do probe "$h" "$ACCESS" "authenticated" "$HBODY"; done

echo "OSN-3 S5 LOCAL REST/RPC PERMISSION PROOF: ALL PASSED"
