...
SELECT 'P1_OK=' || (EXISTS(SELECT 1 FROM public.locations l JOIN public.zones z ON z.id=l.zone_id JOIN public.sectors s ON s.id=z.sector_id WHERE l.id='b1a00001-0066-4a00-8a00-000000000001' AND l.name='Haven'         AND l.physical_role='city' AND l.status='active' AND l.activity_type='none' AND l.x=-50 AND l.y=-30 AND z.name='Wreck Belt'      AND z.status='active' AND s.name='Outer Haven'   AND s.sector_index=1 AND s.status='active'))::int;
SELECT 'P2_OK=' || (EXISTS(SELECT 1 FROM public.locations l JOIN public.zones z ON z.id=l.zone_id JOIN public.sectors s ON s.id=z.sector_id WHERE l.id='b1a00002-0066-4a00-8a00-000000000002' AND l.name='Slagworks' AND l.physical_role='port' AND l.status='active' AND l.activity_type='none' AND l.x=70  AND l.y=-10 AND z.name='Ion Storm Route' AND z.status='active' AND s.name='Crimson Nebula' AND s.sector_index=2 AND s.status='active'))::int;
SELECT 'P3_OK=' || (EXISTS(SELECT 1 FROM public.locations l JOIN public.zones z ON z.id=l.zone_id JOIN public.sectors s ON s.id=z.sector_id WHERE l.id='b1a00003-0066-4a00-8a00-000000000003' AND l.name='Driftmarch'  AND l.physical_role='port' AND l.status='active' AND l.activity_type='none' AND l.x=10  AND l.y=80  AND z.name='Ion Storm Route' AND z.status='active' AND s.name='Crimson Nebula' AND s.sector_index=2 AND s.status='active'))::int;
...
SELECT 'MAP_PORT_NAMES=' || ((public.get_world_map()::text ~ '"name" *: *"Haven"')::int + (public.get_world_map()::text ~ '"name" *: *"Slagworks"')::int + (public.get_world_map()::text ~ '"name" *: *"Driftmarch"')::int);
