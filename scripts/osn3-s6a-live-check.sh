#!/usr/bin/env bash
# OSN-3 S6A — STRICTLY READ-ONLY live post-deploy verification of migration 0060. Read paths only:
#   1) migration list --linked              (0060 applied remotely)
#   2) direct psql catalog query via pooler (AUTHORITATIVE: command_main_ship_space_move identity/owner/
#                                            ACL + authenticated-only EXECUTE; private writer + S4 + S5 +
#                                            S2 helpers still service_role-only; canonical RPC inventory =
#                                            prior + the wrapper; repair still client; S4 cron once; flags;
#                                            cap; movement/receipt counts)
#   3) REST table reads                     (corroborate flags/cap + movement/receipt counts)
# NEVER executes the wrapper / writer / processor / destruction / helpers, NEVER mutates, NEVER changes a
# flag, NEVER seeds. Asserts the live coordinate flag stays FALSE (the deploy mutated no game state).
set -uo pipefail
: "${SUPABASE_DB_PASSWORD:?}" "${URL:?}" "${ANON:?}" "${SRV:?}" "${SUPABASE_ACCESS_TOKEN:?}" "${SUPABASE_PROJECT_ID:?}"
REF="$SUPABASE_PROJECT_ID"
fail() { echo "FAIL: $1"; exit 1; }

echo "=== 1) migration 0060 applied on live ==="
supabase migration list --linked --password "$SUPABASE_DB_PASSWORD" | tee /tmp/migs
awk -F'|' '/20260618000060/{r=$2; gsub(/ /,"",r); if (r=="20260618000060") print "REMOTE_OK"}' /tmp/migs | grep -q REMOTE_OK \
  && echo "ok: migration 0060 is applied on the live database" || fail "0060 not shown applied remotely"
...
echo "=== 3) AUTHORITATIVE catalog inspection (wrapper / ACL / cron / flags / counts) ==="
...
\pset pager off
\echo '--- public wrapper + private engine fn metadata ---'
...
order by p.proname;
-- public wrapper identity + authenticated-only boundary
do $$
declare r record;
begin
  ...
  if r.oid is null then raise exception 'LIVE FAIL: command_main_ship_space_move missing'; end if;
  if r.args <> 'p_target_x double precision, p_target_y double precision, p_request_id uuid' then raise exception 'LIVE FAIL: wrapper signature mismatch (%)', r.args; end if;
  ...
  if r.args ~* 'player' or r.args ~* 'ship' then raise exception 'LIVE FAIL: wrapper accepts a player/ship id'; end if;
  if has_function_privilege('anon',r.oid,'EXECUTE') then raise exception 'LIVE FAIL: anon can EXECUTE the wrapper'; end if;
  if not has_function_privilege('authenticated',r.oid,'EXECUTE') then raise exception 'LIVE FAIL: authenticated cannot EXECUTE the wrapper'; end if;
  if r.proacl is null or exists (select 1 from unnest(r.proacl) a where a::text like '=%') then raise exception 'LIVE FAIL: wrapper PUBLIC-executable'; end if;
  raise notice 'LIVE ok: command_main_ship_space_move — owner=postgres, SECURITY DEFINER, search_path=public, no dynamic SQL, no player/ship id; authenticated-only (anon + PUBLIC denied)';
end $$;
-- private writer + S4 + S5 + 4 S2 helpers still service_role-only
do $$
...
begin
  ...
  if c<>7 then raise exception 'LIVE FAIL: expected 7 server-only space fns, found %', c; end if;
  raise notice 'LIVE ok: private writer + S4 processor + S5 destruction + four S2 helpers remain service_role-only';
end $$;
...
begin
  ...
  if b is distinct from 'false' then raise exception 'LIVE FLAG FAIL: mainship_space_movement_enabled=% (expected false — S6A stays dark)', b; end if;
...
select 'OSN-3 S6A LIVE READ-ONLY CATALOG INSPECTION: ALL PASSED' as result;
SQL
...
echo "OSN-3 S6A LIVE SPOT CHECK (READ-ONLY): ALL PASSED"
