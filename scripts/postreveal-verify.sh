...
reconcile() {  # $1 = OUT
  ...
  ck "$(mval "$OUT" P1_OK)" 1 "Haven Reach active + identity"
  ck "$(mval "$OUT" P2_OK)" 1 "Slagworks Anchorage active + identity"
  ck "$(mval "$OUT" P3_OK)" 1 "Driftmarch Waypost active + identity"
