#!/usr/bin/env node
// gen-0316-combat-five-times-tighter.mjs — emit (or --check) migration 0316.
//
// WHY A GENERATOR: 0316 is mostly config/catalog (which needs none), but it ALSO rewrites ONE hunk
// inside the live combat_create_group_encounter body — the line that turns a hull's WORLD-TRAVEL
// speed into its IN-COMBAT move speed. Re-creating live plpgsql means the generator method: the
// hunk's `old_t` is SLICED verbatim out of the deployed head and `new_t` is CONSTRUCTED from that
// slice by an exactly-once string edit, so even the unchanged words are byte-copies, never retyped
// (the 0303 lesson). The migration then proves the slice is still what is deployed (occurs EXACTLY
// once in pg_get_functiondef), replaces it, and proves the length moved by exactly the hunk delta.
//
//   node scripts/gen-0316-combat-five-times-tighter.mjs          # write the migration
//   node scripts/gen-0316-combat-five-times-tighter.mjs --check  # fail if the file on disk drifted
...
const OUT = MIG('20260618000316_combat_five_times_tighter.sql');
...
// ── HEAD CHECKS: establish that 0301 really is the deployed text of the line this slice takes. ───
// The function's TRUE state is 0301's text (the last `create or replace`) with 0308's TWO and
// 0315's FIVE replace-surgery hunks applied on top. NONE of those seven touch line 754 — 0315's
// nearest hunk starts at 0301:755, the line immediately after — so 0301 is still the byte source
// for this one line. Both checks below are what turn that sentence from a claim into a gate:
// (1) no later TEXTUAL re-create of the function (a `create or replace` after 0301 makes the slice
//     stale outright). `--` line comments are stripped first: 0313 names the function in prose.
// (2) no OTHER replace-rewriter after 0315 — the house `(idx, 'fname',` hunk-row shape. A later
//     migration that merely NAMES the function in a read-only probe or a comment is not drift and
//     must not fail this gate.
{
  const version = (f) => (f.match(/^(\d{14})_/) || [])[1] ?? '';
  const files = readdirSync(MIGDIR).filter((f) => f.endsWith('.sql') && version(f) !== '20260618000316');
  ...
  // 0317 IS EXEMPT BY NAME, not by widening the window — the idiom gen-0315 introduced, for exactly
  ...
  const KNOWN_LATER_REWRITERS = new Set(['20260618000317']);
...
// ── THE ONE HUNK: the world-travel speed -> combat-space speed conversion (0301:754) ──────────────
const H1_OLD = slice(F301, '0301', 754, 754,
  "v_move_speed := coalesce((v_stats->>'speed')::double precision, 1);",
  "v_move_speed := coalesce((v_stats->>'speed')::double precision, 1);");
const H1_NEW =
`          -- 0316 THE UNIT CONVERSION THAT WAS MISSING. v_stats->>'speed' is the hull's WORLD-TRAVEL
...
` + edit(H1_OLD,
  "::double precision, 1);",
  "::double precision, 1)\n" +
  "                          * coalesce(public.cfg_num('combat_player_speed_scale'), 0.2);");
...
const rows = HUNKS.map(([idx, fname, oldT, newT]) => {
  ...
  // PLPGSQL VARIABLE CAPTURE (the 0310 rev.2 / 0314 / 0315 lesson): combat_create_group_encounter
  // declares record variables `pr` and `m`, and plpgsql resolves table-alias-qualified references
  // against variables too — an alias shadowing one of them raises "ambiguous" at FIRST EXECUTION
  // only. Applied to the NEW text; the old text is the deployed head.
  {
...
// Comment-stripping idiom proven by 0305/0306/0308/0310/0314/0315 against this database's settings.
...
if (sql.includes('\r')) {
  throw new Error('generated 0316 carries a CR — the rewrite hunk would never match the deployed body');
}
...
if (check) {
  ...
  try {
    ...
    console.error('0316 CHECK FAIL: migration file is missing — run the generator');
    process.exit(1);
  }
  if (onDisk.replace(/\r\n/g, '\n') !== sql) {
    console.error('0316 CHECK FAIL: the migration on disk is not what the slice generates.');
    console.error('Either the 0301 source drifted or the file was hand-edited. Re-run the generator.');
    process.exit(1);
  }
  console.log('0316 CHECK OK: migration matches the slice taken from 0301.');
} else {
  writeFileSync(OUT, sql);
  console.log(`0316 written: ${OUT}`);
  console.log(`  ${HUNKS.length} hunk sliced from 0301 (nothing retyped; new text constructed from the slice)`);
}
