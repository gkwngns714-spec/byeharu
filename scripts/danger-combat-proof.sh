...
if [ "$MODE" = "selftest" ]; then
  ...
  grep -q "the wave radius is no longer ring + its own range + 1" "$SQL" || fail "harness lacks the CLOSURE spawn-radius self-check (the block SOLVES for its own ring; if a wave stops arriving at ring + its own range + 1 that arithmetic must fail loudly rather than silently stage a different fight)"
  grep -q "solved its spawn radius for"                           "$SQL" || fail "harness lacks the CLOSURE derived-vs-actual wave pin (the radius is solved from a PREDICTED range/speed before the wave exists, so the spawned row must agree with the prediction)"
