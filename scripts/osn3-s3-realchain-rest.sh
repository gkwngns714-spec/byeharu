#!/usr/bin/env bash
# OSN-3 S3 — LOCAL REST/RPC permission proof against the disposable local Supabase PostgREST/GoTrue.
# Proves the public Data-API boundary: the S3 writer mainship_space_begin_move (and, defence-in-depth,
# the four S2 helpers) are REJECTED for the anon key and for a real disposable AUTHENTICATED user JWT,
# so the flag-dark internal writer is unreachable through client REST/RPC routes. The service-role key
# is used ONLY to administer the throwaway user (create/delete) — NEVER as a client-boundary substitute.
...
EMAIL="osn3s3rest.$(date +%s).$RANDOM@example.com"
PASS="Test123456!"
...
echo "OSN-3 S3 LOCAL REST/RPC PERMISSION PROOF: ALL PASSED"
