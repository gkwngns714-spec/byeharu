...
--    service_role-only). Over the FULL applied chain (through 0068) the authenticated/anon EXECUTE surface is
--    the canonical inventory: the prior 16 RPCs PLUS the PORT-LAUNCH-1A authenticated read RPC
--    get_osn_movement_readiness (added by migration 0068) → 17 canonical RPCs.
do $$
declare v_list text; v_expected text :=
  'bootstrap_me,cancel_build_order,command_main_ship_space_move,command_main_ship_space_move_to_location,command_main_ship_space_stop,get_combat_reports,get_my_expedition_preview,get_osn_movement_readiness,'
...
begin
  ...
  raise notice 'ok: client-callable surface is the 17 canonical RPCs (16 + PORT-LAUNCH-1A get_osn_movement_readiness); docking primitive absent from it';
end $$;
