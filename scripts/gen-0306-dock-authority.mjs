#!/usr/bin/env node
// gen-0306-dock-authority.mjs — emit (or --check) migration 0306.
//
// WHY A GENERATOR: 0306 rewrites guard hunks inside two LIVE function bodies. The 0303 lesson —
// "never retype a live function body" — cost a silently-dropped guard. So every `old_t` below is
// SLICED verbatim out of the migration that currently defines the hunk (0216); nothing is retyped
// by hand. The migration then proves the slice is still what is deployed (occurs exactly once in
// pg_get_functiondef) before replacing it, and proves the length moved by exactly the hunk delta.
//
//   node scripts/gen-0306-dock-authority.mjs          # write the migration
//   node scripts/gen-0306-dock-authority.mjs --check   # fail if the file on disk has drifted
...
function slice(from, to, startsWith, endsWith) {
  ...
  if (!first.includes(startsWith)) {
    throw new Error(`0216:${from} expected to contain ${JSON.stringify(startsWith)}, got ${JSON.stringify(first)}`);
  }
  if (!last.includes(endsWith)) {
    throw new Error(`0216:${to} expected to contain ${JSON.stringify(endsWith)}, got ${JSON.stringify(last)}`);
  }
  return text;
}

// ── The seven hunks, sliced from 0216. None of them was touched by 0305 (which patched the SORTIE
//    hunks at 0216:321-339, :419-425 and :588-594) — so each is still verbatim in the deployed body.
const A_RESOLVE = slice(273, 281, '-- ONE statement, ONE snapshot', 'end loop;');
const A_COLOC = slice(315, 320, 'if not (v_gf.status =', 'end if;');
const A_TRANSITION = slice(437, 440, 'select f.current_location_id into v_berth', "location_mode = 'location';");
const A_TAIL = slice(501, 503, 'end if;', "'group_id', v_group);");
const U_DOCKED = slice(426, 432, "if v_gf.status = 'present'", 'end if;');
const D_DOCKED = slice(595, 609, "if v_gf.status = 'present'", 'end if;');
const D_TRANSITION = slice(622, 625, 'select f.current_location_id', "location_mode = 'location'),");

// ── The replacements. Same rule, one definition.
const A_RESOLVE_NEW = `      -- 0306: A GROUP WITH NO SHIPS HAS NO FLEET. This assign is about to be judged against the
...
const A_COLOC_NEW = `        -- 0306: THE ONE DOCKED AUTHORITY. Identical rule ("settled at a real port"), one
...
const A_TRANSITION_NEW = `        select public.fleet_docked_location(f) into v_berth
          from public.fleets f
         where f.id = public.mainship_resolve_fleet(v_ship);`;

const A_TAIL_NEW = `  end if;
...
const U_DOCKED_NEW = `          -- 0306: THE ONE DOCKED AUTHORITY (copy 2 of 3 inside this one function).
...
const D_DOCKED_NEW = `      -- 0306: THE ONE DOCKED AUTHORITY, and the ONE retire idiom. The consume below was the
      ...
      perform public.group_fleet_retire(v_player, v_gf.id);`;

const D_TRANSITION_NEW = `           (select public.fleet_docked_location(f)
              from public.fleets f
             where f.id = public.mainship_resolve_fleet(s.main_ship_id)),`;

const HUNKS = [
  [1, 'assign_ship_to_group', A_RESOLVE, A_RESOLVE_NEW],
  [2, 'assign_ship_to_group', A_COLOC, A_COLOC_NEW],
  [3, 'assign_ship_to_group', A_TRANSITION, A_TRANSITION_NEW],
  [4, 'assign_ship_to_group', U_DOCKED, U_DOCKED_NEW],
  [5, 'assign_ship_to_group', A_TAIL, A_TAIL_NEW],
  [6, 'delete_ship_group', D_DOCKED, D_DOCKED_NEW],
  [7, 'delete_ship_group', D_TRANSITION, D_TRANSITION_NEW],
];
...
// Comment-stripping idiom: chr(10) rather than a '\\n' escape, matching 0305:397-503 — the form
// already proven against this database's standard_conforming_strings setting.
const STRIP = `regexp_replace(prosrc, '--[^' || chr(10) || ']*', '', 'g')`;
...
begin
  select ${STRIP} into v_a
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'assign_ship_to_group';
  select ${STRIP} into v_d
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'delete_ship_group';
...
if (check) {
  ...
  console.log('0306 CHECK OK: migration matches the slices taken from 0216.');
} else {
  writeFileSync(OUT, sql);
  console.log(`0306 written: ${OUT}`);
  console.log(`  ${HUNKS.length} hunks sliced from 0216 (nothing retyped)`);
}
