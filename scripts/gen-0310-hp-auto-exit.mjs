#!/usr/bin/env node
// gen-0310-hp-auto-exit.mjs — emit (or --check) migration 0310.
//
// WHY A GENERATOR: 0310 rewrites ONE hunk inside the live process_combat_ticks body (true head
// 0299:308). The 0303 lesson — "never retype a live function body" — cost a silently-dropped guard
// once already, and the tick is the single most re-emitted function in this chain (reverted work
// FOUR separate times when authors copied a stale migration instead of the deployed text). So the
// `old_t` below is SLICED verbatim out of 0299, the file that last defines the tick; nothing is
// retyped by hand. The migration then proves the slice is still what is deployed (occurs EXACTLY
// once in pg_get_functiondef), replaces it, and proves the length moved by exactly the hunk delta —
// byte parity outside the hunk is a property of the method, not a review promise.
//
//   node scripts/gen-0310-hp-auto-exit.mjs          # write the migration
//   node scripts/gen-0310-hp-auto-exit.mjs --check  # fail if the file on disk drifted
...
const OUT = MIG('20260618000310_hp_auto_exit.sql');
...
// ── The one hunk, sliced from the migration that last defines the tick. ──────────────────────────
// process_combat_ticks — head 0299:308. The insertion site is the tail of the SPATIAL arm's death
// branch (0299:1026-1030): `perform report_create` closing the `if v_hp_after <= 0` block, then the
// per-encounter bookkeeping and the `end if; -- not v_wave_paused` closer. That trailing comment
// makes the slice globally unique in the body (the aggregate arm's death branch closes at a
// different indentation and is not followed by the v_wave_paused closer). 0300..0309 did not
// re-emit the tick (verified: 0301's only mention is a comment; 0305/0306/0308 rewrite OTHER
// functions), so 0299 is the true head.
const T_SITE = slice(F299, '0299', 1026, 1030, 'perform report_create(e.id);', 'end if; -- not v_wave_paused');

// ── The replacement: the site verbatim, with the auto-exit arm inserted IMMEDIATELY AFTER the
// death branch and BEFORE the per-encounter bookkeeping — still inside `if not v_wave_paused`.
// BANNED TOKENS, each load-bearing: no '$' (dollar-quote safety below); no 'elite' (the
// elite-stat-wiring proof greps the RAW tick body, comments included, on every chain); no
// 'movement_create(' / 'cfg_bool(' / 'random(' / 'from locations' (0299 pinned their exact counts
// and this migration re-asserts them unchanged).
const T_SITE_NEW = `          perform report_create(e.id);
        end if;
...
const rows = [[1, 'process_combat_ticks', T_SITE, T_SITE_NEW]].map(([idx, fname, oldT, newT]) => {
  const o = `$h${idx}o$`;
  const n = `$h${idx}n$`;
  ...
  // PLPGSQL VARIABLE CAPTURE: process_combat_ticks declares record variables cu / e / pr / loc /
  // mv-adjacent names, and plpgsql resolves table-alias-qualified references against variables too
  // — a `from combat_units cu` inside this body raised "cu.main_ship_id is ambiguous" at FIRST
  // EXECUTION (never at CREATE, never in any static probe) and the per-row guard silently voided
  // every enabled encounter's tick. The review closed that class for this hunk by sweeping all
  // declarations; this guard is what keeps it closed for the next author. Applied to the NEW text
  // only — the old text is the deployed head, verbatim, not ours to police.
  {
...
if (sql.includes('\r')) {
  throw new Error('generated 0310 carries a CR — the rewrite hunk would never match the deployed body');
}
...
if (check) {
  ...
  try {
    ...
    console.error('0310 CHECK FAIL: migration file is missing — run the generator');
    process.exit(1);
  }
  if (onDisk.replace(/\r\n/g, '\n') !== sql) {
    console.error('0310 CHECK FAIL: the migration on disk is not what the slice generates.');
    console.error('Either the 0299 source drifted or the file was hand-edited. Re-run the generator.');
    process.exit(1);
  }
  console.log('0310 CHECK OK: migration matches the slice taken from 0299.');
} else {
  writeFileSync(OUT, sql);
  console.log(`0310 written: ${OUT}`);
  console.log('  1 hunk sliced from 0299 (nothing retyped)');
}
